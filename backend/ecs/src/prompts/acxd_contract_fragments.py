...
def generative_journey_guidance() -> str:
    ...
    return (
        ...
        "so the conversation returns to the deterministic flow. An exit-condition "
        "edge carries `System.gjConditionIndex eq <i>` (i = the condition's "
        "position); timeout and failure edges carry `node_status`.\n"
        "- `dataCapture` (the contract fills it from the plan's `captures`): "
        "`{\"data\": [{\"name\": \"<slot>\", \"type\": \"slot\", \"required\": true, "
        "\"schema\": {...}}], \"exitEnabled\": true}` makes the journey fill the "
        "flow's attached slots as it talks (live: a free description was "
        "classified onto the slot's enum). When every required value is captured "
        "the journey ENDS and evaluates its edges — neither "
        "`System.gjConditionIndex` nor `node_status eq success` is set for that "
        "exit (live, 2026-09-17), so the captured edge tests the slots themselves: "
        "ONE edge, FIRST in `childNodes`, with `slot <name> exists` for each "
        "captured slot. Without it the runtime logs Error NoMessages and the "
        "caller lands in the fallback flow.\n"
