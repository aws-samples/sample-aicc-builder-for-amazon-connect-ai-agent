"""
System Prompt for Prompt Generator Sub-Agent

This agent generates prompts for Amazon Connect AI agents.
It understands the 3 Native Tools (RETRIEVE, Escalate, Complete),
response format requirements, and multi-language support for voice/chat interactions.

IMPORTANT: MCP tools are automatically injected via {{$.toolConfigurationList}}.
Do NOT include individual tool definitions in the generated prompt.
"""

from .._consistency_rules import SUBAGENT_TERMINOLOGY_AND_ESCALATION

PROMPT_GENERATOR_SYSTEM_PROMPT = SUBAGENT_TERMINOLOGY_AND_ESCALATION + """You are an expert Amazon Connect AI agents prompt designer.
...
try:
    ...
    PROMPT_GENERATOR_SYSTEM_PROMPT += get_clues_suffix()
except ImportError:
